...
// TestParseSetMapSeparateVolumeRecords covers the Backup Exec 'SMP2' Set Map
// layout, where each Set Map Entry's volume records follow the entry as
// separate records in the stream (the entry's LENGTH does not contain them),
// rather than being nested inside the entry's LENGTH per the Type 1 spec.
// parseSetMap must consume those following volume records so the next
// iteration lands on the next Set Map Entry, not on a misread volume.
func TestParseSetMapSeparateVolumeRecords(t *testing.T) {
	// Build two Set Map Entries. Each entry: 91-byte fixed fields, LENGTH=91
	// (no nested volume), numVol=1. A volume record (VOLB) follows each entry
	// as a separate record in the stream.
	mkEntry := func(setNo uint16) []byte {
		e := make([]byte, smeMinSize)
		binary.LittleEndian.PutUint16(e[smeLenOff:], uint16(smeMinSize))
		binary.LittleEndian.PutUint16(e[smeSetNumOff:], setNo)
		binary.LittleEndian.PutUint16(e[smeNumVolOff:], 1) // 1 volume, NOT nested
		binary.LittleEndian.PutUint64(e[smeNumFileOff:], 10)
		e[smeStrTypeOff] = mbcStrASCII
		return e
	}
	vol1 := mbcFDDVolume(1, 1024, "C:", "SYSVOL", "HOST1")
	vol2 := mbcFDDVolume(1, 2048, "D:", "DATA", "HOST2")
	// Patch each volume's LENGTH (FDD common LENGTH at offset 0).
	binary.LittleEndian.PutUint16(vol1, uint16(len(vol1)))
	binary.LittleEndian.PutUint16(vol2, uint16(len(vol2)))

	var raw []byte
	raw = append(raw, mbcLe32(0xDEADBEEF)...) // Media Family ID
	raw = append(raw, mbcLe16(2)...)           // count = 2 Set Map Entries
	raw = append(raw, []byte{0, 0}...)         // pad to smHdrSize (8)
	raw = append(raw, mkEntry(1)...)
	raw = append(raw, vol1...)
	raw = append(raw, mkEntry(2)...)
	raw = append(raw, vol2...)

	sm := parseSetMap(raw)
	if sm == nil {
		t.Fatal("parseSetMap returned nil")
	}
	if len(sm.Entries) != 2 {
		t.Fatalf("entries = %d, want 2 (got: %+v)", len(sm.Entries), sm.Entries)
	}
